...
interface RegistroArchivosProps {
    idActividad: string;
  Visible: boolean;
  idEditar: string;
...
const ComprobanteActividadManual: React.FC<RegistroArchivosProps> = ({ idActividad, Visible, idEditar = "", NombreActividad, Cerrar }) => {
  ...
  const [IdActividad, setIdActividad] = useState<string>(idActividad);
  ...
  const fetchHistorialComprobantes = async () => {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const datosParaGuardar: RegistroArchivoData = {    
        idCliente: idCliente,
      ...
      if (respuesta.ok) {
        showNotification("Comprobante guardado correctamente.", "success");    
        Cerrar("success");
      } else {
  ...
  const HistorialView = () => (
    <>
      <div className="w-full max-w-4xl rounded-2xl bg-gray-100 p-8 space-y-4">
        ...
        {!historialLoading && arvhivos.length === 0 && (
              ...
              <div className="overflow-x-auto p-6">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {arvhivos.map((arvhivo) => {
                      const isDownloading = downloadingId === arvhivo._id;
                      return(
                        <tr key={arvhivo._id}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            {new Date(arvhivo.FechaRegistro).toLocaleDateString('es-ES')}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {new Date(arvhivo.PeriodoFiscal).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 hover:text-blue-800 cursor-pointer">
                            <button 
                                                  onClick={() => handleDownload(arvhivo)}
                                                  className={`
                                                  ...
                                                  `}
                                                  title={`Descargar ${arvhivo.NombreArhivo}`}
                                              ...
                                              >                                                  
                                                  {isDownloading ? (
                                                      ...
                                                      <>
                                                          ...
                                                          <span className="truncate max-w-[100px] inline-block">{arvhivo.NombreArhivo}</span>
                                                      </>
                                                  )}
                                              </button>
                          </td>
